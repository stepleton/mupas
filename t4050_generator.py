...
def program(
    ...
    rev_call_graph: dict[str, set[str]],
  ...
  def const_string_code(symbols: mupas_scopes.SymbolScope):
...
def sta_for(
  ...
  if not isinstance(symbol, mupas_scopes.VariableSymbol): raise RuntimeError(
      ...
      f'ordinal variable, not a {type(symbol)}')
  ...
  # stack before the jump. (A better compiler would be able to insert necessary
  # cleanups in front of the GOTO, but oh well :-P)
  def no_gotos(ast: pascal_parser.AstNode, _):
    if isinstance(ast, pascal_parser.StatementGoto): raise RuntimeError(
        'The 4050 BASIC target does not permit use of GOTO inside FOR loops '
        'whose initial and final control variable values are calculated by '
        'executing a function.')
  # TODO XXX TODO: It's probably necessary to check for Exit as well!
  stack_growth = compiled_initial.stack_growth + compiled_final.stack_growth
  if stack_growth > 0: mupas_descent.depth_first(no_gotos, ast.body, None)
  ...
  if isinstance(symbol.storage, t4050_resources.NumericVariable):
    warnings.warn(
        f'Implementing a FOR loop with loop variable {ast.control_variable} '
        'using a native BASIC FOR loop. Nothing inside this FOR loop is '
        f'allowed to mutate {ast.control_variable} (or BASIC on the 4050 will '
        'abort your program), but right now this compiler is not smart enouth '
        'to detect whether anything might try to change that variable. '
        f"Be careful: don't mess with {ast.control_variable}!")
...
def get_integer_constant(
    ast: pascal_parser.Constant, symbols: mupas_scopes.SymbolScope) -> int:
